...
-- 날짜별 쿼리를 위한 부분 인덱스 (더 효율적)
CREATE INDEX idx_volleyball_games_date_range ON volleyball_games(start_time) 
...
-- 날짜별 경기 조회 예시 (함수 기반 인덱스 대신 범위 쿼리 사용)
-- SELECT * FROM volleyball_games 
-- WHERE start_time >= '2025-09-23T00:00:00+09:00' 
--   AND start_time < '2025-09-24T00:00:00+09:00';

COMMENT ON TABLE volleyball_games IS '배구 경기 정보 테이블 - 크롤링된 V-리그 데이터 저장';
COMMENT ON COLUMN volleyball_games.home_score IS '홈팀 세트 스코어';
COMMENT ON COLUMN volleyball_games.away_score IS '원정팀 세트 스코어';
COMMENT ON COLUMN volleyball_games.league_type IS 'men: 남자부, women: 여자부, university: 대학배구';
COMMENT ON COLUMN volleyball_games.match_status IS '경기 상태: 종료, 1세트, 2세트, LIVE 등';
